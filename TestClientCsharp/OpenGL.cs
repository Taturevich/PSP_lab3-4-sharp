...
namespace TestClientCsharp
{
    ...
    {
        public double lbx; //левый нижний угол ячейки (координата по х)
        public double lby; //левый нижний угол ячейки(координата по y)
    ...
    class OpenGL
    {
        ...
        public bool checkneighbors(int i, int j, int parity)
        {
            ...
            if (i < 10 && cells[i + 1, j].condition != 0 )
            {
                if ((cells[i + 1, j].condition ) == parity)
                {
                    accept = false;
            ...
            if ( i > 0 && j<10 && cells[i, j + 1].condition != 0)
            {
                if ((cells[i, j + 1].condition ) == parity)
                {
                    accept = false;
        ...
        public void ToServer(Socket my_sock)
        {
            byte []buff=new byte[1024]; // буфер для обмена данными
	    while (cnt1 + cnt2 != 100)
	    {
		//прием координат от противника
